...
ssize_t rio_writen(int fd, void *buf, size_t n)/*写入io*/
{
    size_t nleft = n;     /* 要读取多少*/
    ssize_t nwritten;     /* 写入多少 */
    char *bufp = buf;    /* buffer */

    while(nleft > 0){
        if( (nwritten = write(fd ,bufp, nleft) ) < 0 ){    /* 故障*/
...
ssize_t rio_read( rio_t *rp, char *buf, size_t n )/*以流的形式进行读取*/
{
   int cnt;
...
ssize_t rio_readlineb( rio_t *rp, void *buf, size_t maxlen)
{
    int n, rc;
    char c, *bufp = buf;
    for( n = 1; n < maxlen; n++ ){    /* 最大比特数据长度 maxlen - 1 */
